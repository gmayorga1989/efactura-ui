...
@Component({
  selector: 'ts-nota-credito-nueva-page',
  standalone: true,
  imports: [FormsModule, RouterLink, DecimalPipe, TsPageLayoutComponent],
  template: `
    <ts-page-layout
      [title]="t('invoice.creditNoteManualTitle')"
      [subtitle]="t('invoice.creditNoteManualSubtitle')"
      [eyebrow]="t('menu.sales')"
    >
      <div page-actions class="ts-page-actions-toolbar d-flex flex-wrap gap-2">
        <a class="btn btn-light" [routerLink]="['/t', tenant.tenantSlug(), 'ventas', 'notas-credito']">
          {{ t('common.back') }}
        </a>
      ...
      <div class="border rounded p-3 mb-3">
        <h2 class="h6 mb-3">{{ t('invoice.creditNoteSearchInvoice') }}</h2>
        <form class="row g-2 align-items-end" (ngSubmit)="buscar()">
          <div class="col-md-4">
            <label class="form-label" for="nc-num">{{ t('documents.number') }}</label>
            <input
              id="nc-num"
              class="form-control"
        ...
        <p class="text-muted small mt-2 mb-0">{{ t('invoice.creditNoteSearchHint') }}</p>
      </div>

      @if (buscando()) {
        <p class="text-muted">{{ t('common.loading') }}</p>
      } @else if (resultados().length === 0 && busquedaRealizada()) {
        <p class="text-muted mb-0">{{ t('invoice.creditNoteSearchEmpty') }}</p>
...
export class NotaCreditoNuevaPage implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (!this.session.puedeEmitirFacturas()) {
      this.toast.warning(this.t('invoice.listPermissionRequired'));
    }
  }
  ...
  buscar(): void {
    ...
    if (!q) {
      this.toast.warning(this.t('invoice.creditNoteSearchRequired'));
      return;
    }
  ...
  continuar(facturaId: string): void {
    void this.router.navigate(['/t', this.tenant.tenantSlug(), 'facturas', facturaId, 'nota-credito']);
  }
}
